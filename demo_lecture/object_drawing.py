...
def build_triangle_path(center_xy, vertices_xy):
    ...
    Returns:
        path_xy: list of (x,y) in the order:
                 center -> first_corner -> tip -> second_corner -> first_corner -> center
    """
    ...
    path_xy = [
        (cx, cy),      # center (hierhin wird erst bei z_safe, dann bei z_draw gefahren)
...
def build_rectangle_path(center_xy, vertices_xy):
    ...
    We'll tell this story:
        center -> bottom-left -> top-left -> top-right -> bottom-right -> bottom-left -> center
    """
    cx, cy = center_xy
    ...
    path_xy = [
        (cx, cy),  # start at center (top @ z_safe)
        bl,
        tl,
        tr,
        br,
...
def build_hexagon_path(center_xy, vertices_xy):
    ...
    Drawing story for hexagon:
        ...
        center -> v0 -> v1 -> v2 -> v3 -> v4 -> v5 -> v0 -> center
    """
    cx, cy = center_xy
...
def draw_shape_xy(robot, vertices_xy):
    ...
    # 1) Approach above the first vertex at z_safe
    x0, y0 = vertices_xy[0]
    print(f"[DRAW] Approach above first vertex at ({x0:.3f}, {y0:.3f})")
    robot.move(PO_xy(x0, y0, z_safe))

    # 2) Move down to drawing plane
    print(f"[DRAW] Move down to z_draw={z_draw:.3f}")
    robot.move(PO_xy(x0, y0, z_draw))

    # 3) Draw along the vertices at constant z_draw
    for i, (x, y) in enumerate(vertices_xy[1:], start=1):
        print(f"[DRAW] Edge step {i}: to ({x:.3f}, {y:.3f}, {z_draw:.3f})")
        robot.move(PO_xy(x, y, z_draw), True)

    # 4) Retreat back up to z_safe at the last vertex
    x_last, y_last = vertices_xy[-1]
    print(f"[DRAW] Retreat from last vertex at ({x_last:.3f}, {y_last:.3f}) to z_safe={z_safe:.3f}")
    robot.move(PO_xy(x_last, y_last, z_safe))
