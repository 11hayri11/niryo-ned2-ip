...
# ==========================================
CAMERA_INDEX = 0         # 0 = usually the first connected camera - After trying, it works with index 0
FRAME_WIDTH = 1920
FRAME_HEIGHT = 1080
...
cv2.waitKey(1000)           # Changed from cv2.waitKey(1000) to cv2.waitKey(0) so it doesn't close immedeatly - Change it back later so calibration runs unattended! To confirm camera works 1 frame is enough
...
# -----------------------------
# Step 6: Define robot poses (hardcoded baseline – 22 joint-space poses, in radians)
# -----------------------------
robot_poses = [
...
for idx, pose in enumerate(robot_poses, start=1):
    ...
    # --- SB-first, classic fallback ---
    found = False
    corners = None

    # Try the newer Symmetry-Based detector first (if this OpenCV build has it)
    ...
    # Fallback to the classic detector with your flags
    if not found:
        found_cls, corners_cls = cv2.findChessboardCorners(gray, chessboard_size, CB_FLAGS)
        if found_cls and corners_cls is not None:
            found = True
            corners = corners_cls.astype(np.float32)
    print(f"  ↳ Chessboard found: {found}")

    if found:
        # Subpixel refinement for higher precision
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
        corners_subpix = cv2.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)

        # Save corresponding object/image points
        obj_points.append(objp.reshape(-1, 1, 3).astype(np.float32))
        img_points.append(corners_subpix.astype(np.float32))

        # Record end-effector pose (TCP)
        eef_pose = robot.forward_kinematics(pose)
        eef_poses.append(eef_pose)

        # Draw corners for visualization
        vis = frame.copy()
        cv2.drawChessboardCorners(vis, chessboard_size, corners_subpix, True)
        captured_images.append(vis)
        cv2.imshow("Detected Chessboard", vis)
        cv2.waitKey(1)
    else:
        fail_log.append(idx)
        print(f"  [WARN] No chessboard detected at pose {idx}.")
...
rvecs_target2cam = [np.zeros((1, 1, 3), dtype=np.float64) for _ in range(num_sets)]
tvecs_target2cam = [np.zeros((1, 1, 3), dtype=np.float64) for _ in range(num_sets)]
...
rms, K, D, rvecs_target2cam, tvecs_target2cam = cv2.fisheye.calibrate(
    ...
    K=K,
    D=D,
    rvecs=rvecs_target2cam,
    tvecs=tvecs_target2cam,
    flags=flags,
    criteria=criteria
)
...
def niryo_pose_to_matrix(pose):
    """
    Convert a Niryo pose [x, y, z, roll, pitch, yaw] to a 4x4 transformation matrix.
    """
...
print(f"[INFO] Converted {len(T_base_eef)} poses into transformation matrices.")
print("[INFO] Sample translation (m):", np.round(t_gripper2base[0].ravel(), 4))
...
# A) Save captured images (annotated) for auditability
img_count = 0
    ...
    pickle.dump(
        {
            ...
            "eef_poses": eef_poses,                       # TCP poses per image [x,y,z,r,p,y]
            "joint_poses": robot_poses,                   # the 22 joint-space poses used
...
# (Optional) quick summary line for your log
print(
