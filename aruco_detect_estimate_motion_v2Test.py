...
REPROJ_MAX_PX = 0.40    # reject obviously bad PnP fits
AREA_MIN_PX2 = 4500     # reject tiny markers (too far / blurry)
PRINT_VIEW_DEBUG = False

MIN_INLIER_VIEWS = 2          # need at least 2 agreeing views for robust fusion
ABORT_ON_INCONSISTENT = True  # if True: skip snapshot/pick when multiview disagrees
...
DIAG_REPEAT_VIEW_MODE = True   # set True to run the diagnostic test
DIAG_VIEW_IDX = 0               # 0..3 (which OBS view to test)
DIAG_N_SAMPLES = 15             # how many pose estimates at that view
...
def multiview_best_pose(robot, cap, detector, K, dist, marker_len_m,
    ...
    candidates = []
    stamp0 = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]  # one stamp for the whole snapshot
    ...
    for k, q in enumerate(obs_views):
        ...
        # ALWAYS save a raw frame per view (even if read fails, save nothing but log it)
        if not ok:
            if PRINT_VIEW_DEBUG:
                print(f"[VIEW{k}] camera read failed")
            continue

        cv2.imwrite(str(log_dir / f"{stamp0}_view{k}_raw.png"), frame)

        est = estimate_marker_once(frame, detector, K, dist, marker_len_m, target_id)
        if est is None:
            # Save a copy tagged as NOMARKER (optional, but useful)
            cv2.imwrite(str(log_dir / f"{stamp0}_view{k}_NOMARKER.png"), frame)
            if PRINT_VIEW_DEBUG:
                print(f"[VIEW{k}] no marker")
            continue

        # quality gates
        if est["reproj_err"] > REPROJ_MAX_PX or est["area"] < AREA_MIN_PX2:
            cv2.imwrite(str(log_dir / f"{stamp0}_view{k}_GATEFAIL.png"), frame)
            if PRINT_VIEW_DEBUG:
                print(f"[VIEW{k}] gate fail reproj={est['reproj_err']:.3f}px area={est['area']:.0f}px^2")
            continue
        ...
        cv2.imwrite(str(log_dir / f"{stamp0}_view{k}_OK_vis.png"), vis)
        np.savez(str(log_dir / f"{stamp0}_view{k}_T_base_marker.npz"),
    ...
    # ---------- robust inlier selection (translation only) ----------
    P_all = np.stack([c["p"] for c in candidates], axis=0)  # (N,3)
    p_med_all = np.median(P_all, axis=0)
    d_all = np.linalg.norm(P_all - p_med_all[None, :], axis=1)

    keep = d_all <= CONSIST_TOL_M
    kept = [c for c, kflag in zip(candidates, keep) if kflag]

    # Use inliers if we have enough; otherwise fall back to all candidates
    pool = kept if len(kept) >= MIN_INLIER_VIEWS else candidates

    # ---------- fuse translation as median of the pool ----------
    P_pool = np.stack([c["p"] for c in pool], axis=0)
    p_fused = np.median(P_pool, axis=0)
    d_pool = np.linalg.norm(P_pool - p_fused[None, :], axis=1)
    spread_m = float(np.max(d_pool)) if len(d_pool) else 0.0

    inconsistent = spread_m > CONSIST_TOL_M

    # Choose best view (rotation source) from the pool
    best = min(pool, key=lambda c: c["key"])

    # Build fused transform: rotation from best, translation = p_fused
    T_fused = best["T_base_marker"].copy()
    T_fused[:3, 3] = p_fused

    best["T_base_marker"] = T_fused
    best["p_fused"] = p_fused
    best["spread_m"] = spread_m
    best["inconsistent"] = inconsistent
    best["num_views_ok"] = len(candidates)
    best["num_views_used"] = len(pool)
    best["num_views_inliers"] = len(kept)
...
def diag_repeatability_single_view(
    robot, cap, detector, K, dist, marker_len_m,
    T_hand_camera, q6_ref, target_id,
    view_idx, view_joints, n_samples, log_dir, stamp
):
    # Move once to the chosen view
    move_to_joints(robot, view_joints)
    time.sleep(SETTLE_S)
    ...
    for i in range(n_samples):
        ok, frame = cap.read()
        if not ok:
            print(f"[DIAG] view={view_idx} sample={i}: camera read failed")
            continue

        est = estimate_marker_once(frame, detector, K, dist, marker_len_m, target_id)
        if est is None:
            print(f"[DIAG] view={view_idx} sample={i}: no marker")
            continue

        # Same gates as your normal pipeline
        if est["reproj_err"] > REPROJ_MAX_PX or est["area"] < AREA_MIN_PX2:
            print(f"[DIAG] view={view_idx} sample={i}: gate fail "
                  f"reproj={est['reproj_err']:.3f}px area={est['area']:.0f}px^2")
            continue
        ...
        Ps.append(p)
        print(f"[DIAG] view={view_idx} sample={i}: base_xyz=[{p[0]:.4f},{p[1]:.4f},{p[2]:.4f}] "
              f"reproj={est['reproj_err']:.3f}px area={est['area']:.0f}px^2")

        time.sleep(0.05)  # tiny pause so frames differ

    if len(Ps) == 0:
        print(f"[DIAG] view={view_idx}: no valid samples")
        return
    ...
    print(f"[DIAG] view={view_idx}: n={len(Ps)} "
          f"mean=[{mean[0]:.4f},{mean[1]:.4f},{mean[2]:.4f}] "
          f"std=[{std[0]*1000:.1f}mm,{std[1]*1000:.1f}mm,{std[2]*1000:.1f}mm]")

    np.savez(
        str(log_dir / f"{stamp}_DIAG_view{view_idx}.npz"),
        P=P, mean=mean, std=std, joints=joints_now, view_idx=view_idx
    )
...
def live_marker_detection(cap, detector, K, dist, marker_len_m,
    ...
    while True:
        ...
        if key == ord("s"):
            ...
            if DIAG_REPEAT_VIEW_MODE:
                diag_repeatability_single_view(
                    robot, cap, detector, K, dist, marker_len_m,
                    T_hand_camera, q6_ref, target_id,
                    view_idx=DIAG_VIEW_IDX,
                    view_joints=OBS_VIEWS[DIAG_VIEW_IDX],
                    n_samples=DIAG_N_SAMPLES,
                    log_dir=log_dir,
                    stamp=stamp
                )

                # Return to primary obs and do NOT pick/place
            ...
            if USE_MULTIVIEW_ON_SNAPSHOT:
                ...
                if best["inconsistent"] and ABORT_ON_INCONSISTENT:
                    print(f"[SNAPSHOT] Multiview inconsistent (spread={1000*best['spread_m']:.1f}mm) -> skipping pick.")
                    continue
                ...
                print(
                    ...
                    f"views={best['num_views_used']}/{best['num_views_ok']} inliers={best['num_views_inliers']} "
