...
# 4.2 Build pinhole undistortion maps (Charuco intrinsics)
# D_vec comes from Charuco and has length 5 (k1, k2, p1, p2, k3)
D_1xN = D_vec.reshape(1, -1).astype(float)  # shape (1,5) for OpenCV

# Optionally, you could compute an "optimal" new camera matrix:
# newK, roi = cv2.getOptimalNewCameraMatrix(K_mat, D_1xN, (W_act, H_act), alpha=0)
# For now, keep things simple and use K_mat as both input and output.
newK = K_mat.copy()

map1, map2 = cv2.initUndistortRectifyMap(
    cameraMatrix=K_mat,
    distCoeffs=D_1xN,
    R=np.eye(3, dtype=float),   # no rectification rotation
    newCameraMatrix=newK,
    size=(W_act, H_act),
    m1type=cv2.CV_16SC2,
)
print("[INFO] Undistortion maps ready (pinhole).")
